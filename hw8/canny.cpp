...
/* cannyA6_ref.cpp: EECS 222 solution A6 */
	...
	******************************************************************************/
	...
	void main(void)
	{
	   ...
	   for(i=0; i<IMG_NUM; i++)
	   {
	      ...
	      t2 = timeIn.read();
	      //printf("%lf\n",(t2-t).to_double());
	      //sc_time tDelay((t2-t).to_double(),SC_MS);
	      printf("%s: Monitor received frame%3d with       %s delay.\n",t.to_string().c_str(),n+1,(t-t2).to_string().c_str());
	      //printf("%lf\n",(t2-tPrev).to_seconds());
	      printf("%s	%s after previous frame, %lf FPS\n",t.to_string().c_str(),(t2-tPrev).to_string().c_str(),1/(t2-tPrev).to_seconds());  
	      tPrev = t2;
	      /****************************************************************************
...
SC_MODULE(Top)
{
	...
	void before_end_of_elaboration(){
	   ...
	   stimulus.timeOut.bind(q3);
	   monitor.timeIn.bind(q3);
	}
