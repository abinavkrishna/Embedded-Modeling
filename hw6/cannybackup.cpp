...
class Stimulus : public sc_module {
   public:
      ...
      void main() {
         for(int i = 1;i < 21; ++i) {
                       if(i<10){
            sprintf(infilename,"video/EngPlaza00%d.pgm",i);
            }
            else {
            sprintf(infilename,"video/EngPlaza0%d.pgm",i);
            }

            if(VERBOSE || DEBUG) printf("Reading the image %s.\n", infilename);
            if(read_pgm_image(infilename, inImage.image, ROWS, COLS) == 0){
               fprintf(stderr, "Error reading the input image, %s.\n", infilename);
               exit(1);
            }
            if(DEBUG) std::cout<<"Successfull reading image, now sending to platform"<<std::endl;
            outStimulus.write(inImage);
            std::cout<<"Stimulus sent frame "<<i<<" ."<<std::endl;
   ...
   public:
      ...
      void main() {
         for(int i = 1;i<21;++i){
            ...
            IMAGE inImage,edge;
            inImage = inMonitor.read();
            edge = inImage;
            std::cout<<"Monitor received frame "<<i<<" ."<<std::endl;
            if(i<10){
                  sprintf(outfilename,"EngPlaza00%d_edges.pgm",i);
            }
            else {
               sprintf(outfilename,"EngPlaza0%d_edges.pgm",i);
            }
            /* Write out the edge image to a file.*/
            if(VERBOSE) printf("Writing the edge iname in the file %s.\n", outfilename);
               if(write_pgm_image(outfilename, edge.image, ROWS, COLS, "", 255) == 0){
...
class top : public sc_module {
   public:
      ...
      void before_end_of_elaboration() {
         stimulus.outStimulus.bind(Cin1);
         platform.ImgIn.bind(Cin1);
         platform.ImgOut.bind(Cout1);
         monitor.inMonitor.bind(Cout1);
      }
      SC_CTOR (top) : Cin1("Cin1",1), Cout1("Cout1",1), stimulus("stimulus"), platform("platform"), monitor("monitor") {}
};
